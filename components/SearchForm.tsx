...
export const formSchema = z.object({
  location: z.string().min(2, "Must be 2 characters or more long.").max(50),
  dates: z.object({
    from: z
      .date()
      .refine(
        (date) => date >= today,
        `From date must be today (${format(today, "yyyy-MM-dd")}) or later`
      ),
    to: z
      .date()
      .refine(
        (date) => date > today,
        `To date must be after today (${format(today, "yyyy-MM-dd")})`
      ),
  }),
  adults: z
    .string()
